...
class SIE_Module(nn.Module):
    def __init__(self, inChans):
        ...
        chans = 4 * inChans
        self.d11 = nn.Conv3d(inChans, inChans, kernel_size=1, dilation=1, groups=inChans)
        self.d31 = nn.Conv3d(inChans, inChans, kernel_size=3, padding=1 , dilation=1, groups=inChans)
        self.d32 = nn.Conv3d(inChans, inChans, kernel_size=3, padding=2 , dilation=2, groups=inChans)
        self.d34 = nn.Conv3d(inChans, inChans, kernel_size=3, padding=4 , dilation=4, groups=inChans)
        # self.d36 = nn.Conv3d(inChans, inChans, kernel_size=3, padding=6 , dilation=6)
        self.d1 = nn.Conv3d(chans, inChans, kernel_size=1, groups=inChans)
    ...
    def forward(self, x):
        ...
        x34 = self.d34(x)
        # x36 = self.d36(x)
        x = torch.cat((x11, x31, x32, x34), dim=1)
        x = self.d1(x)
        x = torch.sigmoid(x)
        out = x * x0
        out = out + x0
...
class DMFNet_ALL(MFNet): # softmax
    # [128]  Flops:  27.045G  &  Params: 3.88M
    def __init__(self, c=4,n=32,channels=128, groups=16,norm='bn', num_classes=4):
        super(DMFNet_ALL, self).__init__(c,n,channels,groups, norm, num_classes)
...
if __name__ == '__main__':
    ...
    model = DMFNet_ALL(c=4, groups=16, norm='bn', num_classes=4)
